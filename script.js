document.addEventListener("DOMContentLoaded", () => {
    ...
    // Renders quiz questions efficiently
    function renderQuiz() {
            ...
            <div class="question-item">
                ...
                <div class="options-container">
                    ${item.options.map(option => `
                        <div>
                            <input type="radio" id="q${index}_${option.replace(/\s+/g, '')}" name="question${index}" value="${option}" required>
                            <label for="q${index}_${option.replace(/\s+/g, '')}" class="option">${option}</label>
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    // --- Initial Setup ---
    renderQuiz(); // Pre-render the quiz on load for faster startup
    switchScreen("start");
});
